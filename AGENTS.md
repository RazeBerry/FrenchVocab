...
- Sizes are chosen from scales, never invented. `--t-3xs` through `--t-3xl` for type, `--r-xs/sm/md/pill` for radii, `--dur-fast/--dur/--dur-slow` with `--ease` for motion, and `--tap` (44px) as the minimum height of anything tappable. If a value seems to need a step that does not exist, add the step deliberately rather than hardcoding a one-off; the drift this replaced had 31 distinct font sizes, seven of them within 1.3px of each other.
